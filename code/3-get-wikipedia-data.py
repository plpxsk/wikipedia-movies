...
def get_views_by_day(title, begin, end, cols=["article","timestamp","views"]):
  # available columns:
  # [u'access', u'agent', u'article', u'granularity', u'project', u'timestamp', u'views']
...
VIEWS_URL = "https://wikimedia.org/api/rest_v1/metrics/pageviews/per-article/en.wikipedia.org/all-access/user/" 
...
for i in range(len(boxoffice_data)):
  ...
  views_by_day.append(get_views_by_day(title, begin_date_str1, release_date_str1))

  logging.debug("Getting edits, users...")
  API_BASE_PARAMETERS['titles'] = title
  API_BASE_PARAMETERS['rvstart'] = datetime_to_str2_begin(boxoffice_data[i]['release_date'])
  API_BASE_PARAMETERS['rvend'] = datetime_to_str2_release(boxoffice_data[i]['release_date'])
